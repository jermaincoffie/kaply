...
@php
    $navLink = fn($route) => request()->routeIs($route)
...
{{-- ===== SIDEBAR (mobiel) ===== --}}
<aside id="klant-sidebar"
       class="fixed inset-y-0 left-0 z-50 w-64 bg-white dark:bg-neutral-800 border-r border-gray-200 dark:border-neutral-700 flex flex-col transform -translate-x-full transition-transform duration-300 sm:hidden">
    ...
    <div class="px-4 py-3 border-b border-gray-100 dark:border-neutral-700">
        <p class="text-sm font-medium text-gray-800 dark:text-neutral-100">{{ $authUser->name }}</p>
        <p class="text-xs text-gray-400 dark:text-neutral-500 mt-0.5 truncate">{{ $authUser->email }}</p>
    </div>

    {{-- Nav --}}
    <nav class="flex-1 px-3 py-4 space-y-0.5 overflow-y-auto">
        <a href="{{ route('home') }}" class="{{ $navLink('home') }}" onclick="closeSidebar()">
            <svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
                <path stroke-linecap="round" stroke-linejoin="round" d="M21 21l-4.35-4.35M17 11A6 6 0 1 1 5 11a6 6 0 0 1 12 0z"/>
            </svg>
            Kappers zoeken
        </a>
        <a href="{{ route('klant.afspraken') }}" class="{{ $navLink('klant.afspraken') }}" onclick="closeSidebar()">
            <svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
                <path stroke-linecap="round" stroke-linejoin="round" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/>
            </svg>
            Mijn afspraken
        </a>
        <a href="{{ route('klant.account') }}" class="{{ $navLink('klant.account') }}" onclick="closeSidebar()">
            <svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
    ...
    {{-- Thema + Uitloggen --}}
    <div class="px-3 pb-4 pt-2 border-t border-gray-100 dark:border-neutral-700 flex-shrink-0 space-y-1">
        <div class="flex items-center justify-between px-3 py-2">
            <span class="text-sm text-gray-600 dark:text-neutral-400">Thema</span>
            <div class="p-0.5 inline-flex bg-gray-100 dark:bg-neutral-700 rounded-full gap-0.5">
                <button onclick="setTheme('light')" id="theme-light-sidebar"
                        class="size-7 flex justify-center items-center rounded-full text-gray-600 dark:text-neutral-300 transition-colors" title="Licht">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><circle cx="12" cy="12" r="4"/><path d="M12 3v1M12 20v1M3 12h1M20 12h1m-2.636-6.364-.707.707M6.343 17.657l-.707.707m0-11.314.707.707M17.657 17.657l.707.707"/></svg>
                </button>
                <button onclick="setTheme('dark')" id="theme-dark-sidebar"
                        class="size-7 flex justify-center items-center rounded-full text-gray-600 dark:text-neutral-300 transition-colors" title="Donker">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M12 3a6 6 0 0 0 9 9 9 9 0 1 1-9-9Z"/></svg>
                </button>
            </div>
        </div>
...
{{-- Sidebar overlay --}}
<div id="klant-sidebar-overlay" onclick="closeSidebar()"
     class="fixed inset-0 bg-black bg-opacity-50 z-40 hidden sm:hidden"></div>
...
<header class="sticky top-0 z-30 bg-white dark:bg-neutral-800 border-b border-gray-200 dark:border-neutral-700 h-14 flex items-center px-4 sm:px-6 gap-4">
    ...
    <button onclick="openSidebar()" class="flex sm:hidden text-gray-500 hover:text-gray-700 dark:text-neutral-400 dark:hover:text-neutral-200 p-1.5 rounded-lg hover:bg-gray-100 dark:hover:bg-neutral-700 transition-colors flex-shrink-0">
...
<script>
    ...
    function updateThemeButtons(isDark) {
        ['', '-sidebar'].forEach(suffix => {
            const light = document.getElementById('theme-light' + suffix);
            const dark  = document.getElementById('theme-dark' + suffix);
            ...
            if (isDark) dark.className += ' ' + activeClass;
            else light.className += ' ' + activeClass;
        });
    }
    ...
    function openSidebar() {
        document.getElementById('klant-sidebar').classList.remove('-translate-x-full');
        document.getElementById('klant-sidebar-overlay').classList.remove('hidden');
    }

    function closeSidebar() {
        document.getElementById('klant-sidebar').classList.add('-translate-x-full');
        document.getElementById('klant-sidebar-overlay').classList.add('hidden');
    }
</script>
<x-cookie-banner />
