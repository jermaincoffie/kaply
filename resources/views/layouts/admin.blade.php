...
<aside id="sidebar"
    ...
    <nav class="flex-1 px-3 py-4 space-y-0.5 overflow-y-auto">
        ...
        {{-- Kappers --}}
        @php $nieuw = \App\Models\Kapper::where('actief', false)->where('abonnement_status', 'geen')->count(); @endphp
        <a href="{{ route('admin.kappers') }}" class="{{ $linkClass('admin.kappers') }}">
            <svg class="w-4 h-4 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                <path stroke-linecap="round" stroke-linejoin="round" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z"/>
            </svg>
            Kappers
            @if($nieuw > 0)
            <span class="ml-auto min-w-[18px] h-[18px] flex items-center justify-center bg-amber-500 text-white text-[10px] font-bold rounded-full px-1">{{ $nieuw }}</span>
            @endif
        </a>

        {{-- Afspraken --}}
        <a href="{{ route('admin.afspraken') }}" class="{{ $linkClass('admin.afspraken') }}">
            <svg class="w-4 h-4 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
        ...
        {{-- Klanten --}}
        <a href="{{ route('admin.klanten') }}" class="{{ $linkClass('admin.klanten') }}">
            <svg class="w-4 h-4 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
...
<header class="sticky top-0 z-30 lg:ml-64 bg-white dark:bg-neutral-800 border-b border-gray-200 dark:border-neutral-700 h-14 flex items-center px-4 sm:px-6 gap-4">
    ...
    <button onclick="openSidebar()" class="flex lg:hidden text-gray-500 hover:text-gray-700 dark:text-neutral-400 dark:hover:text-neutral-200 p-1.5 rounded-lg hover:bg-gray-100 dark:hover:bg-neutral-700 transition-colors">
        <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
            <path stroke-linecap="round" stroke-linejoin="round" d="M4 6h16M4 12h16M4 18h16"/>
        </svg>
    </button>

    <h1 class="text-sm font-semibold text-gray-700 dark:text-neutral-200">{{ $title ?? 'Admin' }}</h1>
