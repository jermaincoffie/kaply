<div>
            ...
            <div wire:key="afspr-mob-{{ $afspraak->id }}" class="bg-white dark:bg-neutral-800 border border-gray-200 dark:border-neutral-700 rounded-xl px-4 py-3">
                ...
                <div class="flex items-center gap-3 mt-2.5 pt-2.5 border-t border-gray-50 dark:border-neutral-700">
                    @if($isVoltooide)
                        ...
                        <button wire:click="openBewerkReview({{ $afspraak->id }})" class="text-xs text-gray-400 dark:text-neutral-500 hover:text-blue-600 dark:hover:text-blue-400 hover:underline">Bewerk</button>
                        <button wire:click="verwijderEigenReview({{ $afspraak->id }})" wire:confirm="Weet je zeker dat je je beoordeling wilt verwijderen?" class="text-xs text-gray-400 dark:text-neutral-500 hover:text-red-500 hover:underline">Verwijder</button>
                        @else
    ...
    {{-- Bewerk review modal --}}
    @if($bewerkReviewAfspraakId)
    <div class="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
        <div class="absolute inset-0 bg-black/50" wire:click="$set('bewerkReviewAfspraakId', null)"></div>
        <div class="relative w-full sm:max-w-md bg-white dark:bg-neutral-800 sm:rounded-2xl rounded-t-2xl shadow-2xl overflow-hidden">
            <div class="sm:hidden flex justify-center pt-3 pb-1">
                <div class="w-10 h-1 bg-gray-200 dark:bg-neutral-600 rounded-full"></div>
            </div>
            <div class="px-5 pt-4 pb-6">
                <div class="flex items-center justify-between mb-5">
                    <h3 class="text-sm font-semibold text-gray-800 dark:text-neutral-100">Beoordeling bewerken</h3>
                    <button wire:click="$set('bewerkReviewAfspraakId', null)" class="text-gray-400 hover:text-gray-600 p-1.5 rounded-lg hover:bg-gray-100 dark:hover:bg-neutral-700 transition-colors">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
                    </button>
                </div>
                ...
                <div class="flex justify-center gap-2 mb-5">
                    @for($i = 1; $i <= 5; $i++)
                    <button type="button" wire:click="$set('bewerkRating', {{ $i }})"
                            class="transition-transform hover:scale-110 focus:outline-none">
                        <svg class="w-9 h-9 {{ $bewerkRating >= $i ? 'text-amber-400' : 'text-gray-200 dark:text-neutral-600' }} transition-colors"
                             fill="currentColor" viewBox="0 0 20 20">
                            <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
                        </svg>
                    </button>
                ...
                @error('bewerkRating') <p class="text-xs text-red-500 text-center -mt-3 mb-3">{{ $message }}</p> @enderror

                <div class="mb-4">
                    <textarea wire:model="bewerkTekst" rows="3"
                              placeholder="Vertel iets over je ervaring... (optioneel)"
                              class="w-full py-2 px-3 rounded-lg border border-gray-200 dark:border-neutral-700 bg-white dark:bg-neutral-900 text-sm text-gray-800 dark:text-neutral-100 placeholder-gray-400 focus:outline-none focus:border-blue-600 focus:ring-1 focus:ring-blue-600 resize-none"></textarea>
                </div>

                <div class="flex gap-2">
                    <button wire:click="$set('bewerkReviewAfspraakId', null)"
                            class="flex-1 py-2.5 text-sm font-medium rounded-lg border border-gray-200 dark:border-neutral-700 text-gray-600 dark:text-neutral-400 hover:bg-gray-50 dark:hover:bg-neutral-700 transition-colors">
                        Annuleer
                    </button>
                    <button wire:click="slaBewerktReviewOp"
                            class="flex-1 py-2.5 text-sm font-semibold rounded-lg bg-blue-600 text-white hover:bg-blue-700 transition-colors">
                        Opslaan
                    </button>
                </div>
