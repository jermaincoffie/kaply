<div>
    ...
    <div class="bg-white dark:bg-neutral-800 border border-gray-200 dark:border-neutral-700 rounded-xl overflow-hidden">
        ...
        <div class="sm:hidden divide-y divide-gray-50 dark:divide-neutral-700">
            ...
            @php
                $aboLabel = match($kapper->abonnement_status) {
                    'actief'     => ['Betaald',    'bg-blue-50 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400'],
                    'gepauzeerd' => ['Gepauzeerd', 'bg-amber-50 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400'],
                    default      => ['Geen abo',   'bg-gray-100 text-gray-500 dark:bg-neutral-700 dark:text-neutral-400'],
                };
            @endphp
            <div class="px-4 py-3">
                {{-- Rij 1: naam + actieknop --}}
                <div class="flex items-center justify-between gap-3 mb-1">
                    <p class="text-sm font-semibold text-gray-800 dark:text-neutral-100 truncate">{{ str($kapper->salon_naam)->title() }}</p>
                    @if($kapper->actief)
                    <button wire:click="deactiveer({{ $kapper->id }})"
                            class="flex-shrink-0 text-xs font-medium px-2.5 py-1 rounded-lg border border-red-200 dark:border-red-800/40 text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors">
                        Deactiveer
                    </button>
                    @else
                    <button wire:click="activeer({{ $kapper->id }})"
                            class="flex-shrink-0 text-xs font-medium px-2.5 py-1 rounded-lg border border-blue-200 dark:border-blue-800/40 text-blue-600 dark:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-900/20 transition-colors">
                        Activeer
                    </button>
                    @endif
                </div>
                {{-- Rij 2: stad · email --}}
                <p class="text-xs text-gray-400 dark:text-neutral-500 truncate mb-2">{{ str($kapper->stad)->title() }} · {{ strtolower($kapper->user?->email ?? '—') }}</p>
                {{-- Rij 3: badges + no-show --}}
                <div class="flex items-center gap-2 flex-wrap">
                    <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium
                        ...
                        <span class="w-1.5 h-1.5 rounded-full {{ $kapper->actief ? 'bg-green-500' : 'bg-gray-400' }}"></span>
                        {{ $kapper->actief ? 'Actief' : 'Inactief' }}
                    </span>
                    <span class="inline-flex px-2 py-0.5 rounded-full text-xs font-medium {{ $aboLabel[1] }}">
                        {{ $aboLabel[0] }}
                    </span>
        ...
        <table class="hidden sm:table w-full text-sm">
            ...
            <tbody class="divide-y divide-gray-50 dark:divide-neutral-700">
                ...
                <tr class="hover:bg-gray-50/50 dark:hover:bg-neutral-700/20">
                    ...
                    <td class="px-6 py-3.5">
                        @php
                            [$aboTxt, $aboCls] = match($kapper->abonnement_status) {
                                'actief'     => ['Betaald',    'bg-blue-50 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400'],
                                'gepauzeerd' => ['Gepauzeerd', 'bg-amber-50 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400'],
                                default      => ['Geen abo',   'bg-gray-100 text-gray-500 dark:bg-neutral-700 dark:text-neutral-400'],
                            };
