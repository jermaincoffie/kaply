<div>
    {{-- Wachtlijst — alleen desktop, mobiel staat in de medewerker chips rij --}}
    @if($wachtlijst->isNotEmpty())
    <div x-data="{ open: false }" class="hidden lg:block bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-xl mb-6 overflow-hidden">
        ...
        <div x-show="open" x-transition:enter="transition ease-out duration-150" x-transition:enter-start="opacity-0 -translate-y-1" x-transition:enter-end="opacity-100 translate-y-0" class="px-4 pb-4">
            ...
            <p class="text-xs text-amber-600 dark:text-amber-500 mt-3">Bij een annulering worden zij niet automatisch gemaild — bel ze zelf op.</p>
        </div>
    </div>
    @endif
    ...
    <div class="hidden lg:block bg-white dark:bg-neutral-800 border border-gray-200 dark:border-neutral-700 rounded-xl mb-6"
        ...
        {{-- Vandaag --}}
        <div x-show="tab === 'vandaag'" class="grid grid-cols-2 sm:grid-cols-3 divide-x divide-gray-100 dark:divide-neutral-700">
            <div class="p-4">
        ...
        {{-- Maand --}}
        <div x-show="tab === 'maand'" style="display:none" class="grid grid-cols-2 sm:grid-cols-3 divide-x divide-gray-100 dark:divide-neutral-700">
            <div class="p-4">
                <p class="text-xs font-semibold text-gray-400 dark:text-neutral-500 uppercase tracking-wide mb-1.5">Omzet</p>
    ...
    {{-- ===== MOBIELE AGENDA VIEW (alleen zichtbaar op < lg) ===== --}}
    @php
        ...
        foreach ($mobielBlokkeringen as $b) {
    ...
    <div class="lg:hidden mb-6" x-data="{ wachtOpen: false, bijzond: false }">
        ...
        <div class="flex items-center gap-2 pb-1 mb-1">
            ...
            @if($wachtlijst->isNotEmpty())
            <button @click="wachtOpen = !wachtOpen"
                    class="flex-shrink-0 inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold transition-colors"
                    :class="wachtOpen ? 'bg-amber-600 text-white' : 'bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400'">
                <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9"/>
                </svg>
                Wachtlijst ({{ $wachtlijst->count() }})
            </button>
            @endif
        </div>

        {{-- Wachtlijst dropdown mobiel --}}
        @if($wachtlijst->isNotEmpty())
        <div x-show="wachtOpen"
            ...
            <div class="px-4 pt-3 pb-1 space-y-2">
                ...
                <div class="bg-white dark:bg-neutral-800 rounded-xl px-4 py-3">
                    ...
                    @if($wachtende->telefoonnummer)
                    <a href="tel:{{ $wachtende->telefoonnummer }}"
                       class="flex items-center justify-center gap-2 w-full px-3 py-2 rounded-lg bg-amber-600 text-white text-sm font-semibold hover:bg-amber-700 transition-colors">
                        <svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"/>
                        </svg>
                        {{ $wachtende->telefoonnummer }}
            ...
            <p class="text-xs text-amber-600 dark:text-amber-500 px-4 py-2">Bij annulering worden zij niet automatisch gemaild — bel ze zelf op.</p>
        </div>
        @endif
        ...
        <div class="bg-white dark:bg-neutral-800 border border-gray-200 dark:border-neutral-700 rounded-2xl overflow-hidden mb-4">
            ...
            <div x-show="!bijzond" class="flex overflow-y-auto" style="max-height: 520px">
                ...
                <div class="flex-1 relative"
                    ...
                    @php
                        ...
                        $bHeight = max(20, (($bEindMin - $bStartMin) / 60) * $mPxPerUur - 2);
                    ...
                    <button
                        ...
                        @click.stop
                        class="absolute left-1 right-1 rounded-md px-1.5 py-0.5 text-left transition-all z-10
                            {{ $isPauze ? 'bg-gray-200 dark:bg-neutral-600 border border-gray-300 dark:border-neutral-500' : 'bg-gray-200 dark:bg-neutral-700 border border-gray-300 dark:border-neutral-600' }}
                            {{ $bSelected ? 'ring-2 ring-offset-1 ring-gray-400 z-20' : '' }}"
                        style="top: {{ $bTop }}px; height: {{ $bHeight }}px;"
                    >
                        <p class="text-[10px] font-semibold truncate leading-tight {{ $isPauze ? 'text-gray-600 dark:text-neutral-300' : 'text-gray-500 dark:text-neutral-400' }}">
                    ...
                    @php
                        ...
                        $mHeight   = max(24, ($afspraak->dienst->duur_minuten / 60) * $mPxPerUur - 2);
                        $mSelected = $geselecteerdeAfspraak?->id === $afspraak->id;

                        $mIsWalkIn  = !empty($afspraak->walk_in_naam);
                        $mToekomst  = \Carbon\Carbon::parse($afspraak->datum->toDateString() . ' ' . $afspraak->start_tijd)->isFuture();
                        $mKleur = match(true) {
                            $mIsWalkIn && $mToekomst           => 'bg-emerald-100 border-emerald-300 text-emerald-800 dark:bg-emerald-900/20 dark:border-emerald-700 dark:text-emerald-300',
                            $mIsWalkIn                          => 'bg-emerald-500 border-emerald-600 text-white',
                            ...
                            $mToekomst                          => 'bg-blue-100 border-blue-300 text-blue-800 dark:bg-blue-900/20 dark:border-blue-700 dark:text-blue-300',
                    ...
                    <button
                        ...
                        @click.stop
                        class="absolute rounded-md border-l-2 px-1.5 py-0.5 text-left transition-all z-10 {{ $mKleur }} {{ $mSelected ? 'ring-2 ring-offset-1 ring-blue-400 z-20' : '' }}"
                        style="top: {{ $mTop }}px; height: {{ $mHeight }}px; left: calc({{ $mColLeftPct }}% + 2px); width: calc({{ $mColWidthPct }}% - 4px);"
                    >
                        <p class="text-[10px] font-bold truncate leading-tight">{{ substr($afspraak->start_tijd, 0, 5) }}</p>
                        <p class="text-[10px] font-semibold truncate leading-tight">{{ $afspraak->klant_naam }}</p>
                        @if($mHeight > 44)
                        <p class="text-[10px] opacity-75 truncate leading-tight">{{ $afspraak->dienst->naam }}</p>
                        @endif
                    </button>
                    @endforeach
                </div>
            ...
            <div x-show="bijzond" x-transition:enter="transition ease-out duration-150" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" class="overflow-y-auto" style="max-height: 520px">
                ...
                <div class="flex flex-col items-center justify-center py-12 text-center px-4">
                    ...
                    <p class="text-sm text-gray-400 dark:text-neutral-500">Geen bijzonderheden voor deze dag</p>
                </div>
                @else
                <div class="divide-y divide-gray-100 dark:divide-neutral-700">
                    @foreach($mobielActiviteiten as $act)
                    @php
                        [$actIcon, $actBadge, $actBadgeClass] = match($act->type) {
                            'geboekt'     => ['📅', 'Geboekt',     'bg-blue-100 text-blue-600 dark:bg-blue-900/30 dark:text-blue-400'],
                            'walk_in'     => ['🚶', 'Walk-in',     'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400'],
                            'geannuleerd' => ['✖',  'Geannuleerd', 'bg-red-100 text-red-600 dark:bg-red-900/30 dark:text-red-400'],
                            'no_show'     => ['⚠',  'No-show',     'bg-orange-100 text-orange-600 dark:bg-orange-900/30 dark:text-orange-400'],
                            'voltooid'    => ['✓',  'Voltooid',    'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400'],
                            'geblokkeerd' => ['🔒', 'Geblokkeerd', 'bg-gray-100 text-gray-500 dark:bg-neutral-700 dark:text-neutral-400'],
                            default       => ['•',  $act->type,    'bg-gray-100 text-gray-500'],
                        };
                    @endphp
                    <div class="flex items-start gap-3 px-4 py-3">
                        <div class="flex-1 min-w-0">
                            <p class="text-sm text-gray-700 dark:text-neutral-200 leading-snug">{{ $act->tekst }}</p>
                            <p class="text-[11px] text-gray-400 dark:text-neutral-500 mt-0.5">{{ $act->created_at->diffForHumans() }}</p>
                        </div>
                        <span class="flex-shrink-0 text-[10px] font-semibold px-2 py-0.5 rounded-full {{ $actBadgeClass }}">{{ $actBadge }}</span>
                    </div>
                    @endforeach
                </div>
                @endif
